...
accountPutArgs.add_argument(
    "userName", type=str, help="Username is required.", required=True, nullable=False)
accountPutArgs.add_argument(
    "password", type=str, help="Password is required.", required=True, nullable=False)
accountPutArgs.add_argument(
    "email", type=str, help="Email is required.", required=True, nullable=False)
accountPutArgs.add_argument("phoneNumber", type=str,
                            help="Phone number of the user")
accountPutArgs.add_argument("occupation", type=str,
                            help="Occupation of the user")

accountPatchArgs = reqparse.RequestParser()
accountPatchArgs.add_argument("name", type=str, help="Name of the user")
accountPatchArgs.add_argument(
    "phoneNumber", type=str, help="Phone number of the user")
accountPatchArgs.add_argument(
    "occupation", type=str, help="Occupation of the user")
...
class Posts(db.Model):
    ...
    userName = db.Column(db.String(100), db.ForeignKey(
        'accounts.userName'), nullable=False)
    accID = db.Column(db.Integer, db.ForeignKey('accounts.id'), nullable=False)


postPutArgs = reqparse.RequestParser()
postPutArgs.add_argument(
    "title", type=str, help="Title is required.", required=True, nullable=False)
postPutArgs.add_argument(
    "content", type=str, help="Post content can't be empty.", required=True, nullable=False)
postPutArgs.add_argument(
    "accID", type=int, help="Post's account ID", nullable=False)
postPutArgs.add_argument("userName", type=str,
                         help="Post's account userName", nullable=False)
...
likePutArgs.add_argument(
    "accID", type=int, help="Account ID is required", required=True, nullable=False)
likePutArgs.add_argument(
    "userName", type=str, help="UserName is required", required=True, nullable=False)
...
class AccountQueryAll(Resource):
    ...
    def put(self):
        ...
        account = Accounts(name=args['name'], userName=userName, password=args['password'],
                           email=args['email'], phoneNumber=args['phoneNumber'], occupation=args['occupation'])
...
class PostQueryAll(Resource):
    ...
    def put(self):
        ...
        post = Posts(title=args['title'], content=args['content'],
                     accID=args['accID'], userName=args['userName'])
        db.session.add(post)
        db.session.commit()
        return post, 201
...
class PostQueryByUserName(Resource):
    @marshal_with(postFields)
    def get(self, userName):
        result = Posts.query.order_by(
            desc(Posts.id)).filter_by(userName=userName).all()
        if not result:
...
class LikeQueryByPostID(Resource):
    ...
    def put(self, postID):
        ...
        result = Likes.query.filter_by(
            postID=postID, accID=args['accID']).first()
        if result:
            abort(409, message="You already liked this post.")
        like = Likes(
            postID=postID, accID=args['accID'], userName=args['userName'])
        db.session.add(like)
        db.session.commit()
        return like, 201
